...
if __name__ == "__main__":
    ...
    # Suffixes for the POI column to define the table header names.
    ...
    usa_map_path = current_dir.parent.parent / 'data' /'US-map'/ 'rural_countries.geojson'
    #usa_map_path = current_dir.parent.parent / 'data' /'US-map'/ 'urban_countries.geojson' # urban boundaries
    ...
    #for year in [2024]:# code if you want to run a single year
        ...
        # efine the output directory based on distance threshold
        if d0 == 0.25:
            output_dir = f"./results/POI_resutls/{year}_poi_urban_{d0 * 1000}m"
        elif d0 == 1:
            output_dir = f"./results/POI_resutls/{year}_poi_urban_{d0}km"
